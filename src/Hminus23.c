...
static double Hminus23_integrand_x1null_v_def_sign(double v, void *p)
{
   ...
   return FF * SQ(v) / (2 * CU(pa->x3 - v));
}

static double Hminus23_integrand_1_v_def_sign(double v, void *p)
{
   // \theta(x3)\theta(-v)
   integration_par_t *pa = (integration_par_t *)p;
   double FF = Fij(pa->x1, pa->x3 - v, pa->x2 + v, pa->ip, pa->jp, pa->N, pa->M, pa->c_fact);
   if (fabs(FF) < _ZERO_THR_) return 0;
   return FF * pa->x3 * (2 * pa->x2 * (pa->x3 - v) - pa->x3 * (pa->x2 + v)) / (2.0 * SQ(pa->x1) * SQ(pa->x3 - v));
}

static double Hminus23_integrand_2_v_def_sign(double v, void *p)
{
   // \tehta(x2)\theta(v)
   integration_par_t *pa = (integration_par_t *)p;
   double FF = Fij(pa->x1, pa->x3 - v, pa->x2 + v, pa->ip, pa->jp, pa->N, pa->M, pa->c_fact);
   if (fabs(FF) < _ZERO_THR_) return 0;
   return FF * SQ(pa->x2) / (2.0 * SQ(pa->x1) * (pa->x2 + v));
}
...
double Hminus23(int32_t i, int32_t j, int32_t ip, int32_t jp, integration_wrapper_t *IW)
{
   ...
   if (fabs(int_par->x1) < _ZERO_THR_) {
      ...
      if (int_par->x3 > 0 && vmin < 0) {
         ...
         integration(Hminus23_integrand_x1null_v_def_sign, (void *)int_par, vmin, upper, EPS_TOLL, &res1);
         res += res1;
      } else if (int_par->x3 < 0 && vmax > 0) {
         double lower = max2(vmin, 0);
         integration(Hminus23_integrand_x1null_v_def_sign, (void *)int_par, lower, vmax, EPS_TOLL, &res1);
         res -= res1; //!
      }
   } else {
      double res1 = 0;
      if (int_par->x3 > 0 && vmin < 0) {
         double upper = min2(0, vmax);
         integration(Hminus23_integrand_1_v_def_sign, (void *)int_par, vmin, upper, EPS_TOLL, &res1);
         res += res1;
      } else if (int_par->x3 < 0 && vmax > 0) {
         double lower = max2(vmin, 0);
         integration(Hminus23_integrand_1_v_def_sign, (void *)int_par, lower, vmax, EPS_TOLL, &res1);
         res -= res1; //!
      }

      if (int_par->x2 > 0 && vmax > 0) {
         double lower = max2(vmin, 0);
         integration(Hminus23_integrand_2_v_def_sign, (void *)int_par, lower, vmax, EPS_TOLL, &res1);
         res += res1;
      } else if (int_par->x2 < 0 && vmin < 0) {
         double upper = min2(0, vmax);
         integration(Hminus23_integrand_2_v_def_sign, (void *)int_par, vmin, upper, EPS_TOLL, &res1);
         res -= res1; //!
      }
   }
   return res;
