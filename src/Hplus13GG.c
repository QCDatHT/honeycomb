...
static double Hplus13GG_integrand_x2null_v_def_sign(double v, void *p)
{
...
static double Hplus13GG_integrand_1_v_def_sign(double v, void *p)
{
   integration_par_t *pa = (integration_par_t *)p;

   double FF = Fij(pa->x1 - v, pa->x2, pa->x3 + v, pa->ip, pa->jp, pa->N, pa->M, pa->c_fact);
   if (fabs(FF) < (_ZERO_THR_)) return 0;
   return FF * SQ(pa->x1) * (3 * SQ(pa->x1) + 3 * SQ(pa->x3) + 8 * pa->x1 * pa->x3 - v * (3 * pa->x3 + pa->x1)) / (-6 * CU(pa->x2) * SQ(v - pa->x1));
}
static double Hplus13GG_integrand_2_v_def_sign(double v, void *p)
{
   integration_par_t *pa = (integration_par_t *)p;

   double FF = Fij(pa->x1 - v, pa->x2, pa->x3 + v, pa->ip, pa->jp, pa->N, pa->M, pa->c_fact);
   if (fabs(FF) < (_ZERO_THR_)) return 0;
   return FF * SQ(pa->x3) * (3 * SQ(pa->x1) + 3 * SQ(pa->x3) + 8 * pa->x1 * pa->x3 + v * (3 * pa->x1 + pa->x3)) / (-6 * CU(pa->x2) * SQ(v + pa->x3));
}
...
double Hplus13GG(int32_t i, int32_t j, int32_t ip, int32_t jp, integration_wrapper_t *IW)
{
   ...
   if (fabs(int_par->x2) < _ZERO_THR_) {
      ...
      if (int_par->x1 > 0 && vmin < 0) {
         ...
         integration(Hplus13GG_integrand_x2null_v_def_sign, (void *)int_par, vmin, upper, EPS_TOLL, &res1);
         res += res1;
      } else if (int_par->x1 < 0 && vmax > 0) {
         double lower = max2(vmin, 0);
         integration(Hplus13GG_integrand_x2null_v_def_sign, (void *)int_par, lower, vmax, EPS_TOLL, &res1);
         res -= res1; //!
      }
   } else {
      double res1 = 0;
      if (int_par->x1 > 0 && vmin < 0) {
         double upper = min2(0, vmax);
         integration(Hplus13GG_integrand_1_v_def_sign, (void *)int_par, vmin, upper, EPS_TOLL, &res1);
         res += res1;
      } else if (int_par->x1 < 0 && vmax > 0) {
         double lower = max2(vmin, 0);
         integration(Hplus13GG_integrand_1_v_def_sign, (void *)int_par, lower, vmax, EPS_TOLL, &res1);
         res -= res1; //!
      }

      if (int_par->x3 > 0 && vmax > 0) {
         double lower = max2(vmin, 0);
         integration(Hplus13GG_integrand_2_v_def_sign, (void *)int_par, lower, vmax, EPS_TOLL, &res1);
         res += res1;
      } else if (int_par->x3 < 0 && vmin < 0) {
         double upper = min2(0, vmax);
         integration(Hplus13GG_integrand_2_v_def_sign, (void *)int_par, vmin, upper, EPS_TOLL, &res1);
         res -= res1; //!
      }
   }
   return res;
