...
static double Htildeplus12GG_integrand_x3null_v_def_sign(double v, void *p)
{
   ...
   return -FF * (CU(v)) / (6.0 * pow(pa->x1 - v, 4));
}
static double Htildeplus12GG_integrand_1_v_def_sign(double v, void *p)
{
   integration_par_t *pa = (integration_par_t *)p;

   double FF = Fij(pa->x1 - v, pa->x2 + v, pa->x3, pa->ip, pa->jp, pa->N, pa->M, pa->c_fact);
   if (fabs(FF) < (_ZERO_THR_)) return 0;
   return FF * SQ(pa->x1) * (-2 * pa->x1 * pa->x2 + v * (pa->x1 + 3 * pa->x2)) / (6.0 * CU(pa->x3) * SQ(pa->x1 - v));
}
static double Htildeplus12GG_integrand_2_v_def_sign(double v, void *p)
{
   integration_par_t *pa = (integration_par_t *)p;

   double FF = Fij(pa->x1 - v, pa->x2 + v, pa->x3, pa->ip, pa->jp, pa->N, pa->M, pa->c_fact);
   if (fabs(FF) < (_ZERO_THR_)) return 0;
   return FF * SQ(pa->x2) * (-2 * pa->x1 * pa->x2 - v * (pa->x2 + 3 * pa->x1)) / (6.0 * CU(pa->x3) * SQ(pa->x2 + v));
}
...
double Htildeplus12GG(int32_t i, int32_t j, int32_t ip, int32_t jp, integration_wrapper_t *IW)
{
   ...
   if (fabs(int_par->x3) < _ZERO_THR_) {
      ...
      if (int_par->x1 > 0 && vmin < 0) {
         ...
         integration(Htildeplus12GG_integrand_x3null_v_def_sign, (void *)int_par, vmin, upper, EPS_TOLL, &res1);
         res += res1;
      } else if (int_par->x1 < 0 && vmax > 0) {
         double lower = max2(vmin, 0);
         integration(Htildeplus12GG_integrand_x3null_v_def_sign, (void *)int_par, lower, vmax, EPS_TOLL, &res1);
         res -= res1; //!
      }
   } else {
      double res1 = 0;
      if (int_par->x1 > 0 && vmin < 0) {
         double upper = min2(0, vmax);
         integration(Htildeplus12GG_integrand_1_v_def_sign, (void *)int_par, vmin, upper, EPS_TOLL, &res1);
         res += res1;
      } else if (int_par->x1 < 0 && vmax > 0) {
         double lower = max2(vmin, 0);
         integration(Htildeplus12GG_integrand_1_v_def_sign, (void *)int_par, lower, vmax, EPS_TOLL, &res1);
         res -= res1; //!
      }

      if (int_par->x2 > 0 && vmax > 0) {
         double lower = max2(vmin, 0);
         integration(Htildeplus12GG_integrand_2_v_def_sign, (void *)int_par, lower, vmax, EPS_TOLL, &res1);
         res += res1;
      } else if (int_par->x2 < 0 && vmin < 0) {
         double upper = min2(0, vmax);
         integration(Htildeplus12GG_integrand_2_v_def_sign, (void *)int_par, vmin, upper, EPS_TOLL, &res1);
         res -= res1; //!
      }
   }
   return res;
